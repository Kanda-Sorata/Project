 ...
 * 
 * <p>
 * "Each call to PooledConnection.getConnection() must return a newly constructed DataConnection object that exhibits the default DataConnection behavior. Only the most
 * recent DataConnection object produced from a particular PooledConnection is open. An existing DataConnection object is automatically closed, if the getConnection()
 * method of its associated Pooled-DataConnection is called again, before it has been explicitly closed by the application. This gives the application server a way
 * to �take away� a DataConnection from the application if it wishes, and give it out to someone else. This capability will not likely be used frequently in
 * practice."
 * </p>
 * 
 * <p>
 * "When the application calls DataConnection.close(), an event is triggered that tells the connection pool it can recycle the physical database connection. In
 * other words, the event signals the connection pool that the PooledConnection object which originally produced the DataConnection object generating the event can
 * be put back in the connection pool."
 * </p>
 * 
 * <p>
 * "A DataConnection-EventListener will also be notified when a fatal error occurs, so that it can make a note not to put a bad PooledConnection object back in the
 ...
 * an SQLException to the application to notify it of the same error. Note that automatic closing of a DataConnection object as discussed in the previous section
 * does not generate a connection close event."
 * </p>
 * The JDBC 3.0 specification states the same in other words:
 * 
 * <p>
 * "The DataConnection.close method closes the logical handle, but the physical connection is maintained. The connection pool manager is notified that the
 * underlying PooledConnection object is now available for reuse. If the application attempts to reuse the logical handle, the DataConnection implementation throws
 * an SQLException."
 * </p>
 * 
 * <p>
 * "For a given PooledConnection object, only the most recently produced logical DataConnection object will be valid. Any previously existing DataConnection object is
 ...
 * even if a logical DataConnection is open for this PooledConnection, bc the
...
public final class PooledConnectionRegressionTest extends BaseTestCase {
    ...
    /**
     * Test the nb of closeEvents generated when a DataConnection is reclaimed. No
    ...
    protected final class ConnectionListener implements ConnectionEventListener {
        ...
        public void connectionClosed(ConnectionEvent event) {
            ...
            System.out.println(PooledConnectionRegressionTest.this.closeEventCount + " - DataConnection closed.");
        }

        /** */
        public void connectionErrorOccurred(ConnectionEvent event) {
            PooledConnectionRegressionTest.this.connectionErrorEventCount++;
            System.out.println("DataConnection error: " + event.getSQLException());
        }
    }
